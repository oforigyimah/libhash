...
void hex_to_byte_array(const char* hex_string, _uint8* byte_array, int hex_string_length) {
    for (int idx = 0; idx * 2 < hex_string_length; idx++) {
        char c1 = hex_string[idx * 2];
        char c2 = hex_string[idx * 2 + 1];

        _uint8 b1 = (c1 <= '9') ? (c1 - '0') : (c1 - 'a' + 10);
        _uint8 b2 = (c2 <= '9') ? (c2 - '0') : (c2 - 'a' + 10);

        byte_array[idx] = (b1 << 4) | b2;
    }
}
...
void test_sha1() {
	...
	puts("\nTest done.\n");
    _uint8 byte_array[SHA1_HASH_SIZE];
    char * test = "da39a3ee5e6b4b0d3255bfef95601890afd80709";
    hex_to_byte_array(test, byte_array, strlen(test));
    printf("string length :::::::::: %lu\n", strlen(test));
    printf("Hex to byte text ::::::::::");
    for (int j = 0; j < 20; ++j) {
